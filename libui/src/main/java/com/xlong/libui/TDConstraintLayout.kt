...
class TDConstraintLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = -1)
    : ConstraintLayout(context, attrs, defStyleAttr), RatioMeasureDelegate, Checkable,
    RCAttrs {
    ...
    fun setStrokeSize(size: Int) {
        shapeMaker.setStroke(size)
            .apply()
    }

    fun setTDRadius(radius: Float) {
        shapeMaker.setRadius(radius)
            .apply()
    }
